...
func GetIndexes(ctx context.Context, client client.Versioned) (IndicesMetadata, error) {
	decoded := IndicesMetadataWithDefaults{}
	if err := fetch.Fetch(ctx, client, "_all?include_defaults=true", &decoded); err != nil {
		return IndicesMetadata{}, err
	}
	result := IndicesMetadata{}
	...
	for k, v := range decoded {
		mergo.Merge(v.IndexMetadata.Settings, v.Defaults)
		result[k] = v.IndexMetadata
	}
	return result, nil
}
...
type IndicesMetadata = map[IndexName]IndexMetadata
...
type IndicesMetadataWithDefaults = map[IndexName]IndexMetadataWithDefaults

type IndexMetadataWithDefaults struct {
	IndexMetadata
	Defaults struct {
		Index IndexSettings `json:"index"`
	} `json:"defaults"`
